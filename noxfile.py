...
# set environment variables for SAP NWRFCSDK -> set according to your installation
os.environ["SAPNWRFC_HOME"] = "/opt/sap/nwrfcsdk"
os.environ["LD_LIBRARY_PATH"] = "/opt/sap/nwrfcsdk/lib"
...
def tests(session):
    ...
    try:
        ...
        session.run(
            ...
            "--omit=tests/*",
            "--include=src/saltext/sap_nwabap/*",
        )
        ...
        session.run(
            ...
            "--omit=src/saltext/sap_nwabap/*",
            "--include=tests/*",
        )
        try:
            session.run(
                "coverage", "report", "--show-missing", "--include=src/saltext/sap_nwabap/*"
            )
            ...
            #    "--include=src/saltext/sap_nwabap/*,tests/*"
            # )
        finally:
...
def gen_api_docs(session):
    ...
    session.run(
        ...
        "docs/ref/",
        "src/saltext",
        "src/saltext/sap_nwabap/config/schemas",
    )
